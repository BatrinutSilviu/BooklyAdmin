  ...
  template: `
    <div class="space-y-8">
      <div>
        ...
        <p class="text-slate-400 mt-1">Welcome back, {{ auth.currentUser()?.email }}. Here's what's happening with your books today.</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-slate-900/40 border border-slate-800 p-6 rounded-2xl">
          <div class="flex justify-between items-center mb-6">
            <h3 class="font-bold">Book Performance</h3>
        ...
        <div class="bg-slate-900/40 border border-slate-800 p-6 rounded-2xl">
          ...
          <div class="space-y-6">
            <div class="flex items-center justify-between">
              ...
              <span class="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-[10px] font-bold rounded-full uppercase tracking-wider">Active</span>
            </div>
      ...
      <div class="bg-slate-900/40 border border-slate-800 rounded-2xl overflow-hidden">
        <div class="p-6 flex justify-between items-center border-b border-slate-800">
          <h3 class="font-bold">Recent Books</h3>
          <button routerLink="/books/new" class="bg-primary hover:bg-primary/90 text-white text-xs font-bold py-2 px-4 rounded-lg flex items-center gap-2 transition-all">
            ...
            <tbody class="divide-y divide-slate-800">
              @for (book of data.books(); track book.id) {
                <tr class="hover:bg-slate-800/20 transition-colors group cursor-pointer">
                  <td class="px-6 py-4">
                    <span class="font-bold text-sm">{{ book.bookTranslations?.[0]?.title ?? 'Untitled' }}</span>
                  </td>
                  <td class="px-6 py-4 text-sm text-slate-400">{{ book.category_ids?.[0] || 'Uncategorized' }}</td>
                  <td class="px-6 py-4">
                    <span class="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider"
                      [class]="book.status ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-500/10 text-slate-400'">
                      {{ book.status ? 'Active' : 'Inactive' }}
                    </span>
                  </td>
                  <td class="px-6 py-4 text-sm font-mono">{{ book.duration ? book.duration + 'min' : '—' }}</td>
              ...
              @empty {
                <tr>
                  <td colspan="5" class="px-6 py-12 text-center text-slate-500 text-sm">
                    No books yet. <a routerLink="/books/new" class="text-primary hover:underline">Add your first book.</a>
                  </td>
                </tr>
              }
            </tbody>
